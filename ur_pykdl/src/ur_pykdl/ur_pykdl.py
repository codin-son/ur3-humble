...
class ur_kinematics(object):
    """UR Kinematics with PyKDL — ROS2 Humble"""
    ...
    def __init__(self, base_link=None, ee_link=None, robot=None, prefix=None, rospackage=None):
        if robot:
            ...
            pykdl_dir = get_package_share_directory(rospackage_)
            TREE_PATH = os.path.join(pykdl_dir, 'urdf', robot + '.urdf')
            self._ur = URDF.from_xml_file(TREE_PATH)
        else:
            # In ROS2, robot_description comes from a topic or parameter.
            # Use rclpy parameter server via node — caller must pass urdf string
            # or load via robot parameter. Fall back to environment variable.
            import os
            urdf_str = os.environ.get('ROBOT_DESCRIPTION', None)
            if urdf_str:
                self._ur = URDF.from_xml_string(urdf_str)
            else:
                raise RuntimeError(
                    "ur_kinematics: no robot URDF available. "
                    "Set ROBOT_DESCRIPTION env var or pass robot= arg."
                )
    ...
    def print_robot_description(self):
        nf_joints = sum(1 for j in self._ur.joints if j.type != 'fixed')
    ...
    def forward(self, joint_values, tip_link=None):
        ...
        fk_p_kdl.JntToCart(self.joints_to_kdl('positions', joint_values), end_frame)
        return frame_to_list(end_frame)

    def forward_position_kinematics(self, joint_values):
        end_frame = PyKDL.Frame()
        self._fk_p_kdl.JntToCart(self.joints_to_kdl('positions', joint_values), end_frame)
        return frame_to_list(end_frame)
    ...
    def inverse_kinematics(self, position, orientation=None, seed=None):
        ...
        goal_pose = PyKDL.Frame(rot, pos) if (orientation is not None and np.array(orientation).size != 0) else PyKDL.Frame(pos)
